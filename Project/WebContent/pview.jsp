...
<div class="row">
  <div class="col-md-5">
			...
			<div class="col-md-6">
				...
				<p><b>상품 번호 </b>: <%=product.getProductId()%>
				<p><b>매장 명</b> : <%=product.getManufacturer()%>
				<p><b>시장 명</b> : <%=product.getCategory()%>
				<p><b>재고 수</b> : <%=product.getUnitsInStock()%>
				<p><b>원산지</b> : <%=product.getCondition()%>
				<h4><%=product.getUnitPrice()%>원</h4>
 </div>
 <div style="float: right;">
      <form name="addForm" action="add.jsp?id=<%=product.getProductId()%>" method="post">
					<a href="#" class="btn" onclick="addToCart()" style="background-color: rgba(0, 0, 0, 0.1);"> 상품 주문 </a> 
					<a href="cart.jsp" class="btn" style="background-color: rgba(0, 0, 0, 0.1);"> 장바구니 </a>
					<a href="order.jsp" class="btn" style="background-color: rgba(0, 0, 0, 0.1);"> 상품 목록 </a>
  ...
  %>
   <%
    ...
    %>
    	  <a href="pupdate.jsp?id=<%=product.getProductId()%>" class="btn" style="background-color: rgba(0, 0, 0, 0.1);">수정</a>
    	  <a onclick="return confirm('정말로 삭제하시겠습니까?')" href="pdeleteAction.jsp?id=<%=product.getProductId()%>" class="btn" style="background-color: rgba(0, 0, 0, 0.1);">삭제</a>
     <% 
      }
      %>   
  </div>
