...
<div class="row">
  ...
  <td><a href = "nview.jsp?nId=<%= list.get(i).getnId() %>"><%= list.get(i).getnTitle() %></a></td>
  <td><%= list.get(i).getUserId() %></td>
  <td><%= list.get(i).getnDate().substring(0,11) + list.get(i).getnDate().substring(11,13) + "시"
  + list.get(i).getnDate().substring(14,16) + "분" %></td>
  </tr>
  <% 
  }
  %>
  ...
  if(pageNumber != 1){
  %>  
  <a href="news.jsp?pageNumber=<%= pageNumber-1 %>" class="btn" style="background-color: teal; font-size: 10px; color: white;">이전</a>
  <% 
  }if(newsDAO.nextPage(pageNumber+1)){
  %>
  <a href="news.jsp?pageNumber=<%= pageNumber+1 %>" class="btn" style="background-color: teal; font-size: 10px; color: white;">다음</a>
  <%
  }
  %>
  <%
  ...
  %>
   <%
    ...
    %>
    <a href="nwrite.jsp" class="btn" style="float: right; background-color: silver; font-size: 10px;">글쓰기</a>
<% } %>
</div>
</div>
<hr class="featurette-divider">
